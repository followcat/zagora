...
class TestCommands(unittest.TestCase):
    ...
    def test_repl_shorthand_connect_first_form(self):
        self.assertEqual(
            cli._rewrite_repl_shorthand(["-c", "v100", "sync"]),
            ["sync", "-c", "v100"],
        )
        self.assertEqual(
            cli._rewrite_repl_shorthand(["--connect", "v100", "kill", "NT"]),
            ["kill", "-c", "v100", "-n", "NT"],
        )
    ...
    def test_zellij_open_remote_prefers_session_flag(self):
        ...
        self.assertIn("unbind \"Ctrl d\"", argv[2])
        self.assertIn("default_mode \"locked\"", argv[2])
        self.assertIn("locked {", argv[2])
        self.assertIn("SwitchToMode \"Locked\"", argv[2])
    ...
    def test_cmd_sync_prunes_when_two_probes_are_both_empty_and_clean(self):
        args = argparse.Namespace(connect="v100", host="http://s:9876", token=None, transport="auto")
        first = subprocess.CompletedProcess(args=["ssh"], returncode=0, stdout="", stderr="")
        second = subprocess.CompletedProcess(args=["ssh"], returncode=0, stdout="", stderr="")
        current = [{"name": "A", "host": "v100"}]
        with (
            patch("zagora.cli.require_cmd"),
            patch("zagora.cli._server_or_exit", return_value="http://s:9876"),
    ...
    def test_cmd_attach_reconcile_removes_when_ls_returns_nonzero_but_definitive(self):
        args = argparse.Namespace(
            host="http://s:9876",
            token=None,
            transport="auto",
    ...
    def test_cmd_attach_reconcile_removes_when_session_is_exited(self):
        args = argparse.Namespace(
            host="http://s:9876",
            token=None,
            transport="auto",
        ...
        remote_ls = subprocess.CompletedProcess(
            args=["ssh"],
            returncode=0,
            stdout="NT (EXITED - attach to resurrect)\n",
            stderr="",
        )
        with (
            patch("zagora.cli.require_cmd"),
